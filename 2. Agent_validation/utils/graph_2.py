...
def merge_sentiment_items(
    ...
    old_list = parse_sentiments_list(old)
    new_list = parse_sentiments_list(new)

    # If ALSC (new) produced structured items, prefer them.
    if new_list:
        # Deduplicate by (aspect, opinion); prefer decisive (pos/neg) over neutral
        def key(d): return (d.get("aspect",""), d.get("opinion",""))
        allowed = {"positive","negative","neutral"}

        best = {key(d): d for d in old_list}  # start with old
        for d in new_list:
            k = key(d)
            prev = best.get(k)
            cur_sent = d.get("sentiment","neutral")
            cur_sent = cur_sent if cur_sent in allowed else "neutral"
            if not prev:
                best[k] = {**d, "sentiment": cur_sent}
            else:
                prev_sent = prev.get("sentiment","neutral")
                # upgrade neutral → decisive
                if prev_sent == "neutral" and cur_sent in {"positive","negative"}:
                    best[k] = {**d, "sentiment": cur_sent}

        # preserve stable order: old keys first, then new unseen
        order = [key(d) for d in old_list]
        order += [k for k in map(key, new_list) if k not in order]
        return [best[k] for k in order]

    # If new is empty (e.g., ALSC hasn’t run yet), keep whatever old we have
    return old_list
...
def dedup_dict(data):
    """
    Deduplicate a list of dictionaries based on the 'aspect' key.
    """
    unique = []
    seen = set()
    ...
    for item in data:
        ...
        if aspect not in seen:   # skip empty and duplicates
            unique.append(item)
            seen.add(aspect)
    return unique
...
def should_continue_ate(state: ABSAState) -> str:
    ...
    if (state["aspects_counter"] < max_debate_loops) and (state["aspects_decision"] != "accept"):
        return "ATE"  # Continue looping
    else:
        return "OTE"  # Exit the loop
...
def should_continue_ote(state: ABSAState) -> str:
    ...
    if (state["opinions_counter"] < max_debate_loops) and (state["opinions_decision"] != "accept"):
        return "OTE"  # Continue looping
    else:
        return "ALSC"  # Exit the loop
...
def node_alsc_supervision(state: ABSAState) -> ABSAState:
    ...
    sentiments = parse_sentiments_list(out.get("revised_sentiments", []))
    cleaned_sentiments = [item for item in sentiments if item.get('aspect', '').strip() != '']
    cleaned_sentiments = [item for item in cleaned_sentiments if item.get('opinion', '').strip() != '']
    cleaned_sentiments = dedup_dict(cleaned_sentiments)
    state["sentiments"] = cleaned_sentiments
